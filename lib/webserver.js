...
var WebServer = function(appDB, port, env) {
    var self = this;
    ...
    app.get('/api/subscription', function(req, res) {
        appDB.getMarketServer().getMarketSubscription(function(err, result) {
            if (err) {
                res.status(500).send("錯誤:" + err);
            }
            else {
    ...
    app.get('/api/triggerlist', function(req, res) {
        ...
        if (guid == '') {
            ...
            appDB.getTriggerRecords(guid, startDate, endDate, function(err, result) {
                if (err) {
                    res.status(500).send("錯誤:" + err);
                }
                else {

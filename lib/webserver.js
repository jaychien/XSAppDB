...
var WebServer = function(appDB, marketServer, triggerImportSvc, port, env) {
    var self = this;
    ...
    /*
        ...
        /api/subcription => 回傳market上面策略的訂閱人數

        /api/triggercount?guid=..&s=yyyyMMdd&e=yyyyMMdd => 回傳某個策略的觸發次數
        /api/triggerlist?guid=..&n=.. => 回傳某個策略N日以來的觸發記錄

        /api/importtrigger?date=yyyyMMdd => 轉入某一日的觸發記錄
     */

    // Return app object listing (as an array)
    // { type:1, guid: .., name: .., folder: .., execId: .., startTime:.., endTime:.., deleted: .., marketId: .., marketStatus:.., subscription:..}
    //
    ...
    app.get('/api/getenv', function(req, res) {
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({env: self.env}));
    });
    ...
    /*
        triggercount?guid=..&s=yyyyMMdd&e=yyyyMMdd
     */
    app.get('/api/triggercount', function(req, res) {
        var guid = req.param("guid") || '';
        var startDate = moment(req.param("s"), "YYYYMMDD");
        var endDate = moment(req.param("e"), "YYYYMMDD");
        if (!startDate.isValid() || !endDate.isValid() || startDate > endDate) {
            res.status(500).send('startDate/endDate format incorrect. Please pass s=yyyyMMdd, e=yyyyMMdd and make sure s <= e');
        }
        else {
            appDB.getTriggerCount(guid, startDate, endDate, function(err, result) {
                if (err) {
                    res.status(500).send("錯誤:" + err);
                }
                else {
    ...
    /*
        triggerlist?guid=..&s=yyyyMMdd&e=yyyyMMdd
        [ { date: 'YYYYMMDD', time: 'HHmmss', symbol: .., price: .. }]
     */
    app.get('/api/triggerlist', function(req, res) {
        var guid = req.param("guid") || '';
        var startDate = moment(req.param("s"), "YYYYMMDD");
        var endDate = moment(req.param("e"), "YYYYMMDD");
        if (guid == '') {
            ...
            appDB.getTriggerRecords(guid, startDate.toDate(), endDate.toDate(), function(err, result) {
                if (err) {
                    res.status(500).send("錯誤:" + err);
                }
                else {
    ...
    /*
        importtrigger?date=YYYYMMDD
     */
    app.get('/api/importtrigger', function(req, res) {
        var date = moment(req.param("date"), "YYYYMMDD");
        if (!date.isValid()) {
            res.status(500).send('date format incorrect. Please pass date=yyyyMMdd');
        }
        else {
            triggerImportSvc.importTrigger(date.toDate())
                .then(function() {
                    res.setHeader('Content-Type', 'application/json');
                    res.end(JSON.stringify({}));
                })

...
var AppDB = function(options) {
    ...
    this.getAppMarketItemList = function(cb) {
        ...
        self.AppObj.find(function(err, appObjs) {
            if (err != null) {
                ...
                _marketServer.getMarketItems(function(err, result) {
                    if (err != null)
                        cb(err, null);
                    else {
    ...
    /**
     * 回傳某個策略最近N日的觸發記錄, order by date (desc)
     * @param guid 策略的guid
     * @param days 查詢期間
     * @param cb callback(err, result), where result is an array of
     *  {date='yyyyMMdd', time='HHmmss', symbol='..', price='..' }
     */
    this.getTriggerRecords = function(guid, days, cb) {
        guid = guid.toLowerCase();

        var dateNow = moment().endOf('day').toDate();
        var dateStart = moment(dateNow).startOf('day').subtract(days, 'days').toDate();

        self.TriggerRecord.find(
            { $and: [{guid:guid}, {triggerDate : {"$gte":dateStart, "$lt":dateNow}}]},
            null,
            {sort:{triggerDate: -1}},
    ...
    this.getMarketServer = function() {
        return _marketServer;
    };
};

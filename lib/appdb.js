...
var AppDB = function(options) {
    ...
    // AppObj 加上 上架的狀態
    //
    this.AppMarketItem = function(type, guid, name, folder, execId, deleted, marketStatus) {
        this.type = type;
        this.guid = guid;
        this.name = name;
        this.folder = folder;
    ...
    this.parseMarketItems = function(appObjs, marketList) {
        ...
        _.each(appObjs, function(appObj) {
            if (!appObj.deleted) {
                ...
                itemList.push(new self.AppMarketItem(appObj.type, appObj.guid, appObj.name, appObj.folder, appObj.execId, false, marketStatus))
            }
        });
        ...
        _.each(appObjs, function(appObj) {
            if (appObj.deleted) {
                ...
                if (item != null && item.Status == 1) {
                    itemList.push(new self.AppMarketItem(appObj.type, appObj.guid, appObj.name, appObj.folder, appObj.execId, true, self.MARKET_TYPE.OPEN));
                }
            }
        });
